...
import Exceptions.ExcessaoMuitosSeres;
...
public class Territorio extends JPanel {
	...
	public Territorio(String nome) {
		...
		Object[] options = { "Ok", "Sair","Pontuacao Anterior"};
		Object a = JOptionPane.showOptionDialog(null, "Bem-vindo! \n Bouncy Balls e um jogo arcade " +
						...
						"serao mostradas para configuracao do jogo, boa sorte e divirta-se! :)", "Bem-vindo",
	...
	public void jogar()  {
		...
		try {
			if(seres.size() < minimoSeresNoTerritorio){
				...
				catch(ExcessaoMuitosSeres e) { remover_seres(); e.printStackTrace(); }
	...
	private void criar_seres() throws ExcessaoMuitosSeres {

			for (int j = 0; j < maxSpawnSeres; j++) {
				if(seres.size() > 100)
					throw new ExcessaoMuitosSeres();
				switch(ritmo) {
					case 1:
