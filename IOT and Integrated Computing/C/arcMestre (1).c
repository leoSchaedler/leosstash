...
void recebeLight()
{
  Serial.println("RECEBE LUZ");

  masterLuz.listen();
  while (!masterLuz.isListening())
...
void recebeTemp()
{
  Serial.println("RECEBE TEMPERATURA");

  masterTMP.listen();
  while (!masterTMP.isListening())
    { Serial.print(':'); delay(100); }
...
void destratarTemp(){
    ...
    if (tempAtual > 125) tempAtual = mem;
    if (tempAtual < -40) tempAtual = mem;
}
...
void destratarLight(){
    ...
    if (lightAtual > 225) lightAtual = mem;
    if (lightAtual < 0) lightAtual = mem;
}
